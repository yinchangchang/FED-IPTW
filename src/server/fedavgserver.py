...
class FedavgServer(BaseServer):
    ...
    def _create_clients(self, client_datasets):
        ...
        def __create_client(identifier, datasets, cid):
            client = CLINET_CLASS(args=self.args, training_set=datasets[0], test_set=datasets[-1], cid=cid)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=min(int(self.args.K), os.cpu_count() - 1)) as workhorse:
            ...
            for identifier, datasets in TqdmToLogger(
                ...
                clients.append(workhorse.submit(__create_client, identifier, datasets, cid).result())            
                cid += 1
